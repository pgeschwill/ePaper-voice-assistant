...
def update_calendar_panel():
    ...
    INFOSCREEN_HELPER.add_calendar_panel(
        current_image, panel_config=panel_config, calendar_items=calendar_items
    )
...
def update_shopping_list_panel():
    ...
    INFOSCREEN_HELPER.add_panel_with_text(
        draw, panel_config=panel_config, text=shopping_list_content
    )
    current_image.save(current_image.filename)
    if clear_display_sleep:
        DRIVER.clear_display_sleep(current_image, ROTATION)
...
def update_mental_load_panel():
    ...
    INFOSCREEN_HELPER.clear_panel(draw, panel_config)
    INFOSCREEN_HELPER.add_panel_with_text(
        draw, panel_config=panel_config, text=mental_load_content
    )
    current_image.save(current_image.filename)
    if clear_display_sleep:
        DRIVER.clear_display_sleep(current_image, ROTATION)
...
def update_date_info_panel():
    ...
    draw = ImageDraw.Draw(current_image)
    INFOSCREEN_HELPER.clear_panel(draw, panel_config)
    INFOSCREEN_HELPER.add_date_info_panel(
        draw, panel_config=panel_config, formatted_date=formatted_date
    )
    current_image.save(current_image.filename)
    if clear_display_sleep:
        DRIVER.clear_display_sleep(current_image, ROTATION)
