...
class InfoScreenHelper:
    def __init__(self):
        ...
        self.path_to_current_image = os.path.join(
            self.path_to_this_file, "current_image.bmp"
        )
        self.path_to_current_weather_forecast = os.path.join(
            self.path_to_this_file, "current_weather_forecast.png"
        )
    ...
    def add_panel_with_text(self, draw, panel_config, text):
        ...
        draw = self.add_title_with_background(
            draw, panel_config, font_title, padding, panel_x1
        )
        ...
        for index, line in enumerate(lines):
            if (
                panel_y1 + 2 * font_text.size > panel_config["y0"] + panel_height
                and index < len(lines) - 1
            ):
                cut_off_line = line[:-3] + "..."
                draw.text(
                    (panel_config["x0"] + padding, panel_y1),
                    cut_off_line,
                    font=font_text,
                    fill="black",
                )
                warnings.warn(
                    "Text is cut off because it exceeds the given panel height.",
        ...
        self.add_panel_border(
            ...
            panel_config["y0"],
            panel_x1,
            panel_config["y0"] + panel_height + padding,
        )
    ...
    def clear_panel(self, draw, panel_config):
        ...
        draw.rectangle(
            [panel_config["x0"], panel_config["y0"], panel_x1, panel_y1], fill="white"
        )
    ...
    def add_calendar_panel(self, image, panel_config, calendar_items):
        ...
        for index, item in enumerate(calendar_items):
            ...
            draw.text(
                (panel_config["x0"] + 2 * padding + icon_size, panel_y1),
                f"{item['start_date']} {item['start_time']} - {item['end_date']} {item['end_time']}",
                font=font_text,
                fill="black",
            )
            draw.text(
                (
                    panel_config["x0"] + 2 * padding + icon_size,
                    panel_y1 + font_text.size,
                ),
                f"{item['content']}",
                font=font_text,
                fill="black",
    ...
    def add_weather_panel(self, image, panel_config, weather_data, forecast_data):
        ...
        weather_icon = Image.open(
            os.path.join(self.path_to_icons, weather_data["icon"] + ".png")
        )
        ...
        wind_text = (
            f" {weather_data['wind']['speed']}m/s [{weather_data['wind']['dir']}]"
        )
        image.paste(
            resized_humidity_icon,
            (panel_config["x0"] + padding, panel_y1),
            resized_humidity_icon,
        )
        draw.text(
            (panel_config["x0"] + padding + font_title.size, panel_y1),
            humidity_text,
            font=font_title,
            fill="black",
        )
        image.paste(
            resized_wind_icon,
            (panel_config["x0"] + padding, panel_y1 + font_title.size),
            resized_wind_icon,
        )
        draw.text(
            (
                panel_config["x0"] + padding + font_title.size,
                panel_y1 + font_title.size,
            ),
            wind_text,
            font=font_title,
            fill="black",
        ...
        draw.text(
            (panel_config["x0"] + padding, panel_y1 + 2 * font_title.size),
        ...
        icon_x = (
            panel_config["x0"]
            + (panel_x1 - panel_config["x0"] + padding - icon_size) // 2
        )
        image.paste(resized_weather_icon, (icon_x, panel_y1), resized_weather_icon)
        draw.text(
            (icon_x + icon_size + 5 * padding, panel_y1 - 2 * padding),
            cur_temp_text,
            font=self.fonts["robotoBlack54"],
            fill="black",
        )
        draw.text(
            (icon_x + icon_size + 5 * padding, panel_y1 + 2 * font_title.size),
            min_max_text,
            font=font_title,
            fill="black",
        )

        weather_forecast_output_filename = os.path.join(
            os.path.dirname(image.filename), "current_weather_forecast.png"
        )
        self.create_weather_forecast_graph(
            panel_config=panel_config,
            forecast_data=forecast_data,
            output_filename=weather_forecast_output_filename,
        )
    ...
    def create_weather_forecast_graph(
        self, panel_config, forecast_data, output_filename=""
    ):
        ...
        fig, (ax1, ax2) = plt.subplots(
            ...
            figsize=(
                panel_config["width"] * px * 1.2,
                (panel_config["height"]) / 1.8 * px,
            ),
        )
        ...
        for i, icon in enumerate(forecast_data["icon"]):
            ...
            ax1.imshow(
                ...
                aspect="equal",
                origin="upper",
                extent=[left_x, right_x, 0, img_height],
            )
        ...
        time_range = (
            np.arange(current_hour, current_hour + num_hours, step=3) % num_hours
        )
    ...
    def add_date_info_panel(self, draw, panel_config, formatted_date):
        ...
        title_x = (
            panel_config["x0"]
            + (panel_x1 - panel_config["x0"] + padding - date_width) // 2
        )
        title_y = panel_config["y0"] + padding
    ...
    def add_title_with_background(
        self, draw, panel_config, font_title, padding, panel_x1
    ):
        ...
        draw.rectangle(
            [
                ...
                panel_config["y0"],
                panel_x1,
                panel_config["y0"] + font_title.size + 2 * padding,
            ],
            fill=tuple(panel_config["title_background_color"]),
        )
        ...
        title_x = (
            panel_config["x0"]
            + (panel_x1 - panel_config["x0"] + padding - title_width) // 2
        )
        title_y = panel_config["y0"] + padding
        draw.text(
            (title_x, title_y), panel_config["title"], font=font_title, fill="black"
        )
