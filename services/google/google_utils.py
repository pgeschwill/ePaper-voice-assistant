...
def get_google_creds():
    ...
    path_to_client_secret = os.path.join(
        PATH_TO_THIS_FILE, "auth", "client_secret.json"
    )
...
def get_document_content(document_id):
    ...
    return document["body"]["content"][1]["paragraph"]["elements"][0]["textRun"][
        "content"
    ]
...
def parse_calendar_events(events, timeformat):
    ...
    for event_key in sorted(events.keys()):
        start = events[event_key]["start"].get(
            "dateTime", events[event_key]["start"].get("date")
        )
        end = events[event_key]["end"].get(
            "dateTime", events[event_key]["end"].get("date")
        )
