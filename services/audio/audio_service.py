...
VOLUME_PERCENT = config["audio"]["initial_volume_percent"]
...
def play_response():
    ...
    file_name_to_play = os.path.join(
        PATH_TO_RESPONSE_FILES, response_files[file_index_to_play]
    )
...
def generate_response():
    ...
    params = {"text": phrase, "voice": "de_DE/thorsten_low"}
...
def generate_wav():
    ...
    output_filename = (
        f"{os.path.normpath(os.path.join(PATH_TO_RESPONSE_FILES, filename))}"
    )
    if not output_filename.startswith(
        PATH_TO_RESPONSE_FILES
    ) or not output_filename.endswith(".wav"):
        return jsonify(
            error="Illegal path", message="Illegal path for output file.", success=False
        ), 400
    params = {"text": phrase, "voice": "de_DE/thorsten_low"}
    response = requests.get(MIMIC3_URL, params=params)
    with open(output_filename, "wb") as out_file:
...
def play_wav():
    filename = request.args.get("filename")
    full_filename = (
        f"{os.path.normpath(os.path.join(PATH_TO_RESPONSE_FILES, filename))}.wav"
    )
    if not full_filename.startswith(PATH_TO_RESPONSE_FILES) or not os.path.exists(
        full_filename
    ):
        return jsonify(
            error="Not Found",
...
def play(filename):
    private_set_output_volume(VOLUME_PERCENT)
